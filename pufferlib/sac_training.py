    ...
    with ExitStack() as stack:
        ...
        if args.resume:
            ...
            for i, env in enumerate(envs):
                history[i] = env.reset(seed=args.seed + i + args.num_envs * int(resets[i]))[0]
        ...
        while not stopping and decisions < args.steps and time.time() < args.stop_at.timestamp():
            ...
            with torch.no_grad():
                if decisions < args.warmup:
                    action = np.stack([np.random.randint(n, size=args.num_envs) for n in limits], -1)
                else:
                    action = collector.sample(torch.from_numpy(flat))[0][0].numpy()
            results = [env.step(a) for env, a in zip(envs, action)]
            following = np.stack([row[0] for row in results])
            rewards = np.asarray([row[1] for row in results], dtype=np.float32)
            terminal = np.asarray([row[2] for row in results], dtype=np.float32)
            ...
            replay.add(torch.as_tensor(flat, device=args.device), torch.as_tensor(action, device=args.device),
                       ...
                       torch.as_tensor(next_history.reshape(args.num_envs, -1), device=args.device),
            ...
            for i, row in enumerate(results):
                if row[2] or row[3]:
                    episode_file.write(json.dumps(dict(seed=args.seed + i + args.num_envs * int(resets[i]),
                        reward=float(returns[i]), decisions=int(lengths[i]), terminated=bool(row[2]))) + '\n')
                    episode_file.flush()
                    completed += 1
                    resets[i] += 1
