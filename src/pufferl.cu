...
__global__ void sample_logits(
    ...
    if (is_continuous) {
        ...
        total_log_prob = ConditionalSample(logits + logits_base,
            actions + idx * num_atns, &state);
        for (int h = 0; h < num_atns; ++h) {
            env_actions[idx * num_atns + h] = actions[idx * num_atns + h];
        }
#else
        int logits_offset = 0;
...
static void env_setup(PuffeRL* p, VecEnv* vec, Dict* vk, Dict* ek) {
    ...
    while (agents_created < total_agents) {
        ...
        if (envs[num_envs].num_agents <= 0
                || envs[num_envs].num_agents > total_agents - agents_created) {
            fprintf(stderr, "environment agent count does not fit vec.total_agents\n");
            exit(1);
        }
    ...
    for (int i = 0; i < num_envs; i++) {
        ...
        if (buf_agents > apb) {
            fprintf(stderr, "environment crosses vector buffer boundary\n");
            exit(1);
        }
        vec->env_counts[buf]++;
...
__global__ void transpose_102(precision_t* dst, const precision_t* src,
        int A, int B, int C, int buffer_rows, int learner_rows) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    int learners = B / buffer_rows * learner_rows;
    int total = A * learners * C;
    if (idx >= total) {
        return;
    }
    int a = idx / (learners * C);
    int rem = idx % (learners * C);
    int b = rem / C;
    int c = rem % C;
    int physical = b / learner_rows * buffer_rows + b % learner_rows;
    dst[b * A * C + a * C + c] = src[(a * B + physical) * C + c];
}

#if !defined(PRECISION_FLOAT)
__global__ void transpose_102(float* dst, const float* src, int A, int B, int C, int buffer_rows, int learner_rows) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    int learners = B / buffer_rows * learner_rows;
    int total = A * learners * C;
    if (idx >= total) {
...
static void train_epoch_gpu(PuffeRL* pufferl, RolloutBuf src, int slot,
    ...
    int buffer_rows = pufferl->vec->agents_per_buf;
    int learner_rows = pufferl->vec->policy_layout[1];
    transpose_102<<<grid_size(T * B * obs_size), BLOCK_SIZE, 0, stream>>>(
        rollouts->observations.data, src.observations.data, T, B, obs_size, buffer_rows, learner_rows);
    transpose_102<<<grid_size(T * B * num_atns), BLOCK_SIZE, 0, stream>>>(
        rollouts->actions.data, src.actions.data, T, B, num_atns, buffer_rows, learner_rows);
    transpose_102<<<grid_size(T * B), BLOCK_SIZE, 0, stream>>>(
        rollouts->logprobs.data, src.logprobs.data, T, B, 1, buffer_rows, learner_rows);
    transpose_102<<<grid_size(T * B), BLOCK_SIZE, 0, stream>>>(
        rollouts->rewards.data, src.rewards.data, T, B, 1, buffer_rows, learner_rows);
    transpose_102<<<grid_size(T * B), BLOCK_SIZE, 0, stream>>>(
        rollouts->terminals.data, src.terminals.data, T, B, 1, buffer_rows, learner_rows);
    transpose_102<<<grid_size(T * B), BLOCK_SIZE, 0, stream>>>(
    ...
    if (hypers->prioritized_replay) {
        puff_advantage<<<grid_size(n_rows), BLOCK_SIZE, 0, stream>>>(
            ...
            hypers->gamma, hypers->gae_lambda, 1, 1, n_rows, T, pufferl->current_gamma);
        ReplayWeights<<<grid_size(n_rows), BLOCK_SIZE, 0, stream>>>(
            replay->advantages.data, replay->probabilities.data, n_rows, T, hypers->prio_alpha);
        ReplayCDF<<<1, 1, 0, stream>>>(replay->probabilities.data, replay->cdf.data, n_rows);
    }
    for (int mb = 0; mb < total_minibatches; ++mb) {
        int dest_off = (mb * mb_segs) % n_rows;
        RolloutBuf selected = *rollouts;
        Prec state = pufferl->train_state;
        TrainGraph graph = pufferl->train_buf;
        if (hypers->prioritized_replay) {
            long* rng_offset = pufferl->rng_offset + hypers->num_buffers;
            ReplaySample<<<grid_size(Nmb), BLOCK_SIZE, 0, stream>>>(
                replay->indices, replay->importance.data, replay->cdf.data,
                replay->probabilities.data, n_rows, Nmb, replay->beta,
                pufferl->seed, rng_offset);
            ReplayAdvance<<<1, 1, 0, stream>>>(rng_offset, Nmb);
            GatherReplay(replay, *rollouts, state, stream);
            selected = replay->batch;
            state = replay->state;
            dest_off = 0;
            graph.mb_importance = replay->importance.data;
        }
        graph.mb_obs = slice_rows(selected.observations, dest_off, Nmb);
        graph.mb_actions = slice_rows(selected.actions, dest_off, Nmb);
        graph.mb_logprobs = slice_rows(selected.logprobs, dest_off, Nmb);
        graph.mb_terminals = slice_rows(selected.terminals, dest_off, Nmb);
...
void puf_dashboard_print(Ini* ini, PuffeRL* p, Dict* log, int epoch) {
    ...
    DictItem* utilization = dict_find(log, "util/gpu_percent");
    if (utilization) {
        snprintf(gpu, sizeof(gpu), "%3.0f%%", utilization->value);
    } else {
...
static EvalResult eval_loop(Ini* ini, PuffeRL* p, int mode, int verbose,
    ...
    while (true) {
        ...
        result.games = (int)n;
        const char* output = puf_ini_get_str(ini, "base", "eval_result_path");
        if (strcmp(output, "None") != 0) {
            DictItem* length = dict_find(&el, "env/episode_length");
            dict_set(&el, "evaluation_decisions", length ? length->value * n : p->collected_steps);
            dict_set(&el, "policy_evaluations", p->collected_steps);
            dict_set(&el, "time", time(NULL));
            checkpoint::WriteMetrics(output, &el);
        }
        dict_clear(&el);
        return result;
    }
...
// Signals request a save at the next completed update, outside the handler.
static volatile sig_atomic_t train_stop_requested = 0;
static void RequestTrainStop(int) {
...
TrainResult run_train(Ini* ini, TrainContext* ctx) {
    ...
    if (ctx->artifact_owner) {
        ...
        mkdir_p(log_dir);
        checkpoint::WriteIni(std::filesystem::path(checkpoint_dir) / "config.ini", ini);
    }

    PuffeRL* pufferl = create_pufferl(ini, ctx);
    const char* resume = puf_ini_get_str(ini, "base", "resume_training_state");
    bool resuming = strcmp(resume, "None") != 0;
    if (!resuming) {
        char resolved_model[4096];
        const char* model = puf_checkpoint_path_key(ini, "load_model_path", resolved_model, sizeof(resolved_model));
        if (model) {
    ...
    bool stopped = false;
    if (ctx->artifact_owner && save_state) {
        char initial[4096];
    ...
    for (long epoch = pufferl->epoch; epoch < train_epochs; epoch++) {
        ...
        dict_set(&new_log, "fresh_decisions", pufferl->collected_steps);
        dict_set(&new_log, "stopped", stopped);
        dict_set(&new_log, "pid", getpid());
        float current_gamma;
        ...
        DictItem* matches = dict_find(&new_log, "env/matches");
        ...
        if (ctx->artifact_owner) {
            ...
            checkpoint::WriteMetrics(std::filesystem::path(checkpoint_dir) / "progress.ini", &new_log);
        }
    ...
    long bot_games = !stopped && use_selfplay ? puf_ini_get(ini, "selfplay", "eval_bot_games") : 0;
    int max_opp = use_selfplay ? puf_ini_get(ini, "selfplay", "eval_pool_size") : 0;
    long pool_games = !stopped && use_selfplay ? puf_ini_get(ini, "selfplay", "eval_games") : 0;
    ...
    long eval_episodes = stopped ? 0 : puf_ini_get(ini, "base", "eval_episodes");
...
int main(int argc, char** argv) {
    ...
    if (resume != "None") {
        ...
        checkpoint::Require(strcmp(puf_ini_get_str(&ini, "base", "env_name"),
            PUFFER_ENV_NAME) == 0, "checkpoint belongs to a different environment");
    }
    if (model) {
