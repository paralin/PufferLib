...
struct TrainGraph {
    const float* mb_importance; // Optional replay correction, one weight per trajectory.
...
__global__ void cache_imp_and_v(
    ...
    if (logstd.data) {
        ...
        new_lp = ConditionalLogProb(logits + logits_base,
            actions + idx * NUM_ATNS, logps + at_base);
#else
...
__global__ void puff_advantage(const precision_t* values,
        ...
        int num_steps, int horizon, const float* current_gamma = NULL) {
    if (current_gamma) {
        gamma = *current_gamma;
    }
